# SimpleRNN Benchmarking Script
# Includes Baseline, Optimized, and Quantized + Pruned models
...
# --- SimpleRNN Model Definitions ---

# Baseline SimpleRNN model
def build_baseline_simplernn(input_shape):
...
# Optimized SimpleRNN: smaller RNN, added pooling
def build_optimized_simplernn(input_shape):
    model = tf.keras.Sequential([
        tf.keras.layers.Reshape((1, input_shape), input_shape=(input_shape,)),
...
# Quantization wrapper using TFLite
def quantize_model(model):
    converter = tf.lite.TFLiteConverter.from_keras_model(model)
    converter.optimizations = [tf.lite.Optimize.DEFAULT]

    # Fixes for TensorList-related ops
    converter.target_spec.supported_ops = [
        tf.lite.OpsSet.TFLITE_BUILTINS,
        tf.lite.OpsSet.SELECT_TF_OPS  # allow fallback to TF ops
    ]
...
# Train and profile baseline model
baseline = train_model(build_baseline_simplernn(input_dim), X_train, y_train, EPOCHS, BATCH_SIZE)
...
# Train and profile optimized model
optimized = train_model(build_optimized_simplernn(input_dim), X_train, y_train, EPOCHS, BATCH_SIZE)
...
# Prune, retrain, quantize
pruned = apply_pruning(build_optimized_simplernn(input_dim))
...
results = pd.DataFrame(
    ...
    index=["Baseline SimpleRNN", "Optimized SimpleRNN", "Quantized + Pruned SimpleRNN"]
)

save_results(results.to_dict(orient="index"), "tensorflow_simplernn_comparison.csv")
print(results)
